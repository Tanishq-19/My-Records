...
//    void save(int date, String item1, String item2, String item3, String item4, String item5, String item6, String item7, String item8, int item2rs, int item1rs,  int item3rs, int item4rs, int item5rs, int item6rs, int item7rs, int item8rs, int item1qty, int item2qty, int item3qty, int item4qty, int item5qty, int item6qty, int item7qty, int item8qty, int item1ttl, int item2ttl, int item3ttl, int item4ttl, int item5ttl, int item6ttl, int item7ttl, int item8ttl, int itemtotal, int id);
    ...
    @Query("INSERT INTO notes (content, expenditure, stock, date, total_dayend, total_exp) Values ('item1, item2, item3, item4, item5, item6, item7, item8, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00,', 'item1, item2, item3, item4, 00, 00, 00, 00, Nil, Nil, Nil, Nil', 'item1, item2, item3, item4, item5, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00,', '00-00-0000', '00', '00')")
    void create();

    @Query("SELECT * FROM notes")
    List<Note> getAll();

    @Query("UPDATE notes SET content = :content, expenditure = :expenditure, stock = :stock, date = :date, total_dayend = :total_dayend, total_exp = :total_exp WHERE id = :id")
    void save(String content, String expenditure, String stock, String date,String total_dayend, String total_exp, int id);
